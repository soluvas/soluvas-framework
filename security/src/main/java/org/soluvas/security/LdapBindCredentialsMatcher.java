...
public class LdapBindCredentialsMatcher implements CredentialsMatcher {
	...
	public boolean doCredentialsMatch(AuthenticationToken token,
		...
		if (token instanceof UsernamePasswordToken) {
			...
			log.debug("Skipping LDAP authentication for {} because of AutologinToken", token.getPrincipal());
			return true;
		} else {
			throw new SecurityException("Unsupported AuthenticationToken " + token.getClass().getName());
		}
	}
