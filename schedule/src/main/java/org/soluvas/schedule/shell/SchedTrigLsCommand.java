...
public class SchedTrigLsCommand extends ExtCommandSupport {
	...
	protected Integer doExecute() throws Exception {
		...
		System.out.println(ansi().render("@|negative_on %3s|%-35s|%-15s|%-8s|%-16s|%-16s|%-16s|%-16s|@",
		...
		for (final TriggerKey it : triggerKeys) {
			...
			if (trigger instanceof CronTrigger) {
				triggerExpr = "cron " + ((CronTrigger) trigger).getCronExpression();
				...
				triggerExpr = "calendar " + ciTrigger.getRepeatInterval() + ciTrigger.getRepeatIntervalUnit();
				...
				triggerExpr = "daily " + dtiTrigger.getRepeatInterval() + dtiTrigger.getRepeatIntervalUnit() + " " + dtiTrigger.getDaysOfWeek();
			} else if (trigger instanceof SimpleTrigger) {
				triggerExpr = "simple " + ((SimpleTrigger) trigger).getRepeatInterval() + "ms";
			} else {
				triggerExpr = trigger.toString();
			}
			final JobDataMap jobDataMap = trigger.getJobDataMap();
			System.out.println(ansi().render("@|bold,black %3d||@%-35s@|bold,black ||@%-15s@|bold,black ||@%-8s@|bold,black ||@%-16s@|bold,black ||@%-16s@|bold,black ||@%-16s@|bold,black ||@%-16s",
			...
			System.out.println(ansi().render("@|bold,black    ||@%-35s@|bold,black ||@%-24s@|bold,black ||@%s", triggerExpr, trigger.getJobKey().getName(), ImmutableMap.copyOf(jobDataMap)));
		}
