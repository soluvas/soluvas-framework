...
public class SchedTrigLsCommand extends ExtCommandSupport {
	...
	protected Integer doExecute() throws Exception {
		...
		System.out.println(ansi().render("@|negative_on %3s|%-40s|%-15s|%-8s|%-16s|%-16s|%-16s|%-16s|@",
		...
		for (final TriggerKey it : triggerKeys) {
			...
			System.out.println(ansi().render("@|bold,black %3d||@%-40s@|bold,black ||@%-15s@|bold,black ||@%-8s@|bold,black ||@%-16s@|bold,black ||@%-16s@|bold,black ||@%-16s@|bold,black ||@%-16s",
